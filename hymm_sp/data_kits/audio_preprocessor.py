...
def encode_audio(wav2vec, audio_feats, fps, num_frames=129):
    if fps == 25:
        ...
        start_ts = [0]
        step_ts = [2]
    num_frames = min(num_frames, 2000)  # 80s at 25fps (per forensic audit: 400 was 16s cap, 750 was 30s, 2000 is full 80s)
    ...
    # Pad audio_feats so last frames have enough tokens
    needed = num_frames * 2 + 14
    if audio_feats.shape[1] < needed:
        pad = torch.zeros_like(audio_feats[:, :needed - audio_feats.shape[1]])
